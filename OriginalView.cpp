...
void OriginalView::draw()
{
	...
	if ( m_pDoc->m_ucBitmap ) 
	{
		...
		//cout << m_nWindowWidth << " "  << m_nWindowHeight << endl;
		//cout << drawWidth << " " << drawHeight << endl;
		int diff_y = m_nWindowHeight - drawHeight;
		//cout << 'd' << diff_y << endl;
		if (x_pos > 0 && x_pos < drawWidth && y_pos > diff_y && y_pos < m_nWindowHeight) {
			glPointSize(5.0);
			glColor3f(1, 0, 0);
			glBegin(GL_POINTS);
...
void OriginalView::markerPos(int x, int y) {
	x_pos = x;
	y_pos = y;
	refresh();
}
